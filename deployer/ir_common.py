...
# generated.py's typed dataclasses by protocol/hydrate.py) in, shell
# script text out. Every node this module touches is one of protocol.
# generated.InfraHostNode/OvnLsNode/OvnLrpNode — never a raw dict.
# That's deliberate: the protocol (src/protocol.ts's ArkType schema) is
# the stabilizing factor for the TypeScript/Python boundary, and it only
# actually stabilizes anything if the Python side uses the typed nodes
# it produces — a dict access typo or a stale-shape assumption here
# would otherwise fail silently (KeyError deep inside script emission,
# or worse, a None slipping through) instead of loudly, at hydration,
...
def _emit_kernel_router(host_id: str, nodes: list[pt.Model], action: Action) -> list[str]:
    lines: list[str] = []
                    ...
                    lines.append(
                        _sh(
                            netns_ops.netns_exec_argv(
                                linux_net_ops.delete_link_argv(real_name), netns
                            )
                        )
                    )
...
def _emit_iface_bindings(host_name: str, nodes: list[pt.Model], action: Action) -> list[str]:
    ...
    if action == "create":
        ...
        if mappings:
            lines.append(
                _sh(ovs_ops.set_external_id_argv("ovn-bridge-mappings", ",".join(mappings)))
            )
    else:
        any_bound = False
        for domain, iface in bindings:
...
def _emit_host(host_node: pt.InfraHostNode, nodes: list[pt.Model], action: Action) -> list[str]:
    ...
    if action == "delete":
        ...
        # (2026-08-18).
        lines.extend(_emit_kernel_router(host_node.id, nodes, action))
        lines.extend(_emit_iface_bindings(host_name, nodes, action))
        return lines

    lines = _emit_kernel_router(host_node.id, nodes, action)
    lines.extend(_emit_iface_bindings(host_name, nodes, action))
...
def _emit_host_script(host_node: pt.InfraHostNode, nodes: list[pt.Model], action: Action) -> str:
    host_name = host_node.key.host
    body = _emit_host(host_node, nodes, action)
    lines = [
        "#!/bin/sh",
